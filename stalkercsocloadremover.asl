...
init
{
    ...
    if (game.ProcessName == "xrEngine")
    {
        ...
        var PromptSig = new SigScanTarget("C6 ?? ?? ?? ?? ?? ?? 48 8D ?? ?? ?? ?? ?? 44 8B ?? 44 8B");
        var PromptLocation = scanner.Scan(PromptSig);
        var PromptAddr = PromptLocation + 2;
        var PromptRIP = memory.ReadValue<int>(PromptAddr);
        var PromptRIPAddr = PromptLocation + 7 + PromptRIP;
...
split
{
    return vars.CurMap.Changed && vars.CurMap.Current != "" && vars.CurMap.Old != "" && settings["autosplitter"] || vars.End.Current == "final";
}
isLoading
{
